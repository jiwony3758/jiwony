...
검색엔진은 사이트맵을 읽고 사이트를 효율적으로 크롤링한다.<br/>
...
<CodeBlock language="javascript" title="/app/sitemap/route.ts">
{`
export function GET() {
  const date = new Date();
...
<CodeBlock language="javascript" title="/next.config.js">
{`
const nextConfig = \{
  async rewrites() \{
    return [
...
<CodeBlock language="javascript" title="/app/robots/route.ts">
{`
export function GET() \{
  return new Response(
    \`User-agent: *
